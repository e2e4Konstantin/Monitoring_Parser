...
from files_features import create_abspath_file, file_exists
from files_features import output_message_exit
from public import generate_column_names, clear_string, get_float
from config import CellPosition
...
class ExcelControl:
    ...
    def file_in_use(self) -> bool:
        """Проверяет использование файла другим приложением."""
...
class SourceData:
    ...
    def set_column_names(self):
        ...
        self.df.columns = generate_column_names(self.df.shape[1])
    ...
    def find_cell_with_cleaning(self, target_text: str) -> CellPosition | None:
        ...
        for row in range(0, self.row_max + 1):
            for column in self.df.columns:
                value = clear_string(self.get_cell_value(row, column))
